...
1. 启动项目并进入“创作设置”。
2. 粘贴在 Kimi 开放平台创建的 API Key。
3. 选择 Key 所属的国内站或国际站。
4. 点击“验证并保存”。验证通过后即可在生成弹窗选择 Kimi 创作模式。

Key 验证通过后只保存在本机 `.secrets/kimi-api-key` 文件中，文件权限为仅当前用户可读写。完整 Key 不会进入 SQLite、浏览器存储、日志、导出包、API 响应或 Git。
...
Kimi 在本项目中负责文本策划、内容生成、图片提示词和可选审核。连接检测通过官方模型列表和余额接口完成，不生成内容。正式生成仍需要每次明确确认费用。

图片生成当前使用零费用本地占位图与手工上传；Kimi 的多模态能力是图片理解，不是图片生成。音频也没有接入任何付费服务。
